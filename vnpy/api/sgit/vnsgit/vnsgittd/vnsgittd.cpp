...
void TdApi::processTask()
{
	try
	{
		while (this->active)
		{
			...
			case ONRSPQRYDEFERFEERATE:
			{
				this->processRspQryDeferFeeRate(&task);
				break;
			}

			case ONRTNDEFERFEERATE:
			{
				this->processRtnDeferFeeRate(&task);
				break;
			}

			case ONRSPQRYDEFERMIDTRADE:
			{
				this->processRspQryDeferMidTrade(&task);
				break;
			}

			case ONRTNDEFERMIDTRADE:
			{
				this->processRtnDeferMidTrade(&task);
				break;
			}

			case ONRTNPOSITIONNETTING:
			{
				this->processRtnPositionNetting(&task);
				break;
			}
...
void TdApi::processRspQryDepthMarketData(Task *task)
{
	...
	if (task->task_data)
	{
		...
		data["CenterChangeNo"] = task_data->CenterChangeNo;
		data["CodecPrice"] = task_data->CodecPrice;
		data["PriceTick"] = task_data->PriceTick;
		data["VolumeMultiple"] = task_data->VolumeMultiple;
		delete task_data;
	}
...
void TdApi::processRspQryDeferFeeRate(Task *task)
{
	gil_scoped_acquire acquire;
	dict data;
	if (task->task_data)
	{
		CThostDeferFeeRateField *task_data = (CThostDeferFeeRateField*)task->task_data;
		data["ExchangeID"] = toUtf(task_data->ExchangeID);
		data["InstrumentID"] = toUtf(task_data->InstrumentID);
		data["EffectDate"] = toUtf(task_data->EffectDate);
		data["PayDirection"] = task_data->PayDirection;
	...
	this->onRspQryDeferFeeRate(data, error, task->task_id, task->task_last);
};

void TdApi::processRtnDeferFeeRate(Task *task)
{
	gil_scoped_acquire acquire;
	dict data;
	if (task->task_data)
	...
	this->onRtnDeferFeeRate(data);
};

void TdApi::processRspQryDeferMidTrade(Task *task)
{
	gil_scoped_acquire acquire;
	dict data;
	if (task->task_data)
	{
		CThostDeferMidAppTradeField *task_data = (CThostDeferMidAppTradeField*)task->task_data;
		data["BrokerID"] = toUtf(task_data->BrokerID);
		data["ClientID"] = toUtf(task_data->ClientID);
		data["InvestorID"] = toUtf(task_data->InvestorID);
		data["InstrumentID"] = toUtf(task_data->InstrumentID);
	...
	this->onRspQryDeferMidTrade(data, error, task->task_id, task->task_last);
};

void TdApi::processRtnDeferMidTrade(Task *task)
{
	gil_scoped_acquire acquire;
	dict data;
	if (task->task_data)
	...
	this->onRtnDeferMidTrade(data);
};

void TdApi::processRtnPositionNetting(Task *task)
{
	gil_scoped_acquire acquire;
	dict data;
	if (task->task_data)
	{
		CThostFtdcPositionNettingField *task_data = (CThostFtdcPositionNettingField*)task->task_data;
		data["BrokerID"] = toUtf(task_data->BrokerID);
		data["InstrumentID"] = toUtf(task_data->InstrumentID);
		data["InvestorID"] = toUtf(task_data->InvestorID);
		data["Direction"] = task_data->Direction;
		data["HedgeFlag"] = task_data->HedgeFlag;
		data["PositionNetting"] = task_data->PositionNetting;
		data["Price"] = task_data->Price;
		data["MarginNetting"] = task_data->MarginNetting;
		delete task_data;
	}
	this->onRtnPositionNetting(data);
};
...
public:
	...
	void onRspQryDeferFeeRate(const dict &data, const dict &error, int reqid, bool last) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, TdApi, onRspQryDeferFeeRate, data, error, reqid, last);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
	...
	void onRtnDeferFeeRate(const dict &data) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, TdApi, onRtnDeferFeeRate, data);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
	...
	void onRspQryDeferMidTrade(const dict &data, const dict &error, int reqid, bool last) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, TdApi, onRspQryDeferMidTrade, data, error, reqid, last);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
	...
	void onRtnDeferMidTrade(const dict &data) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, TdApi, onRtnDeferMidTrade, data);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
	...
	void onRtnPositionNetting(const dict &data) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, TdApi, onRtnPositionNetting, data);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
