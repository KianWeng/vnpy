...
void MdApi::OnRtnDeferDeliveryQuot(CThostDeferDeliveryQuot *pQuot)
{
	...
	if (pQuot)
	{
		CThostDeferDeliveryQuot *task_data = new CThostDeferDeliveryQuot();
		*task_data = *pQuot;
		task.task_data = task_data;
	}
	this->task_queue.push(task);
...
void MdApi::processRtnDeferDeliveryQuot(Task *task)
{
	...
	if (task->task_data)
	{
		CThostDeferDeliveryQuot *task_data = (CThostDeferDeliveryQuot*)task->task_data;
		data["InstrumentID"] = toUtf(task_data->InstrumentID);
...
public:
	...
	void onRtnDeferDeliveryQuot(const dict &data) override
	{
		try
		{
			PYBIND11_OVERLOAD(void, MdApi, onRtnDeferDeliveryQuot, data);
		}
		catch (const error_already_set &e)
		{
			cout << e.what() << endl;
